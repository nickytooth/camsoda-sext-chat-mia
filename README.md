# Victoria — AI Girlfriend Sexting Chat
...
- **Always-open persona** — every reply is generated by Grok. Victoria is a single forward persona (`personas/victoria2.yaml`); there is no SFW/NSFW model switch.
...
- **Idle re-engagement** — if the user goes quiet while still connected, Victoria may send one spontaneous follow-up.
...
personas/
└── victoria2.yaml         # The active always-open persona
...
```bash
createdb victoria
# or: psql -c "CREATE DATABASE victoria;"
```
...
| `DATABASE_URL` | ✅ | PostgreSQL connection string (default: `postgresql://postgres:postgres@localhost:5432/victoria`) |
| `SINGLE_PERSONA_FILE` | | Active persona YAML (default: `personas/victoria2.yaml`) |
...
|----------|--------|-------------|
| `/ws/chat` | WebSocket | Real-time bidirectional chat; sends Victoria's opening on first connect |
